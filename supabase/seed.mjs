...
// Remove stale rows (e.g. old sample projects) that aren't in the list above,
// then upsert the real ones. Full sync: DB ends up matching this file.
const keep = rows.map((r) => r.slug);
const { error: delError } = await supabase
  .from("projects")
  .delete()
  .not("slug", "in", "(" + keep.map((s) => '"' + s + '"').join(",") + ")");
if (delError) explain("Cleanup", delError.message);

const { error } = await supabase.from("projects").upsert(rows, { onConflict: "slug" });
if (error) explain("Seed", error.message);

console.log("Seeded " + rows.length + " projects (stale rows removed)");
